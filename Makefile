...
kube-apply:
	...
	kubectl apply -f target/kubernetes/exchange-simle
	kubectl apply -f target/kubernetes/exchange-enterprise
	kubectl apply -f target/kubernetes/calculator

expose:
	kubectl port-forward $$(kubectl get pods -o=name | grep exchange-simle-deployment) 9091 || : &
...
cleanup:
	$(info Be sure minikube config is the active)
	kill -9 $$(ps x | grep "port-forward pod/exchange-simle-deployment" | head -1 | cut -d' ' -f 1)
	...
	@kubectl delete -f target/kubernetes/exchange-simle || :
